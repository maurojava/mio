...
// Model for MicroMarket entity
    models.MicroMarket = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/mio/webresources/micromarket/",
        idAttribute: 'zipCode',
        defaults: {
            areaLength: "",
            areaWidth: "",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('zipCode');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8080/mio/webresources/micromarket/';
            }
    ...
    // Collection class for MicroMarket entities
    models.MicroMarketCollection = Backbone.Collection.extend({
        model: models.MicroMarket,
        url: "http://localhost:8080/mio/webresources/micromarket/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.MicroMarketCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-micromarket-list-itemis template identifier for item
                        templateName: '#tpl-micromarket-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.micromarket = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.micromarket,
                    // tpl-micromarket-details is template identifier for chosen model element
                    templateName: '#tpl-micromarket-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.MicroMarket();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-micromarket-details is a template identifier for chosen model element
                templateName: '#tpl-micromarket-details',
