...
// Model for DiscountCode entity
    models.DiscountCode = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/mio/webresources/discountCode/",
        ...
        sync: function (method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8080/mio/webresources/discountCode/';
            }
    ...
    models.DiscountCodeCollection = Backbone.Collection.extend({
        ...
        url: "http://localhost:8080/mio/webresources/discountCode/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.DiscountCodeCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-discountcode-list-itemis template identifier for item
        ...
        details: function (id) {
            if (this.collection) {
                this.discountcode = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    ...
                    // tpl-discountcode-details is template identifier for chosen model element
                    templateName: '#tpl-discountcode-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.DiscountCode();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-discountcode-details is a template identifier for chosen model element
                templateName: '#tpl-discountcode-details',
